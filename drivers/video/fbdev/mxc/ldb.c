...
static int g_spl_mode;
static int g_dual_mode;
static struct lvds_option_data g_option[ARRAY_SIZE(imx6q_ldb_buses)];
...
static inline int ldb_option_set(int value, int *opt)
{
	if(value > 0xFFFF)
		return 0;
...
static inline bool ldb_option_is_set(int opt)
{
	return ((opt & 0x10000) ? true : false);
}

static inline int ldb_option_get(int opt)
{
	return (opt & 0xFFFF);
}

static int ldb_option_handle_lvds(char *opt)
{
	int i;
	char *s, *p;
	int temp;
	...
	s = opt + 4;
	i = simple_strtoul(s, NULL, 0);
	if(i >= imx6q_ldb_info.bus_mux_num)
		return 0;

	if((s = strchr(opt, ':')) == NULL)
		return 0;
	s++;

	if(!strncmp(s, "off", 3)) {
		ldb_option_set(0, &g_option[i].status);
		return 1;
	} else if(!strncmp(s, "on", 2))
		ldb_option_set(1, &g_option[i].status);
	...
	while((p = strsep(&s, ",")) != NULL) {
		...
		if(!strncmp(p, "index=", 6)) {
			temp = simple_strtoul(p + 6, NULL, 0);
			ldb_option_set(temp, &g_option[i].index);
		} else if(!strncmp(p, "width=", 6)) {
			temp = simple_strtoul(p + 6, NULL, 0);
			ldb_option_set(temp, &g_option[i].width);
		} else if(!strncmp(p, "mapping=", 8)) {
			if(strncmp(p + 8, "spwg", 4))
				ldb_option_set(LVDS_BIT_MAP_SPWG, &g_option[i].mapping);
			else if(!strncmp(p + 8, "jeida", 5))
				ldb_option_set(LVDS_BIT_MAP_JEIDA, &g_option[i].mapping);
		}
	}
	return 1;
}
...
#ifndef MODULE
/*
 * "ldb=lvds1:off"		-- disable lvds1
 * "ldb=lvds0:index=1,width=18,mapping=spwg" -- index 1,18 bit, spwg standard
 * "ldb=split-mode" lvds split mode
 * "ldb=dual-mode" lvds dual mode
 */

static int __init ldb_option_setup(char *opt)
{
	int ret = 0;

	if(!strncmp(opt, "split-mode", 10))
		ret = ldb_option_set(1, &g_spl_mode);
	else if(!strncmp(opt, "no-split-mode", 13))
		ret = ldb_option_set(0, &g_spl_mode);
	else if(!strncmp(opt, "dual-mode", 9))
		ret = ldb_option_set(1, &g_dual_mode);
	else if(!strncmp(opt, "no-dual-mode", 12))
		ret = ldb_option_set(2, &g_dual_mode);
	else if(!strncmp(opt, "lvds", 4))
		ret = ldb_option_handle_lvds(opt);

	return ret;
}
__setup("ldb=", ldb_option_setup);
#endif
static int ldb_probe(struct platform_device *pdev)
{
	...
	if(ldb_option_is_set(g_spl_mode))
		ldb->spl_mode = ldb_option_get(g_spl_mode);
	else
	...
	if(ldb_option_is_set(g_dual_mode))
		ldb->dual_mode = ldb_option_get(g_dual_mode);
	else
	...
	for_each_child_of_node(np, child) {
		...
		if(ldb_option_is_set(g_option[i].status)) {
			if(!ldb_option_get(g_option[i].status))
				continue;
		...
		if(ldb_option_is_set(g_option[i].width))
			data_width = ldb_option_get(g_option[i].width);
		else
		...
		if(ldb_option_is_set(g_option[i].mapping))
			mapping = ldb_option_get(g_option[i].mapping);
		else
		...
		if(ldb_option_is_set(g_option[i].index))
			ret = of_get_videomode(child, &chan->vm,
					ldb_option_get(g_option[i].index));
		else
